...
export function StatCards({ stats, orientation = 'horizontal' }: StatCardsProps) {
  const items = [
  ...
  return (
    <div className={cn(
      "flex gap-3 sm:gap-4",
      orientation === 'horizontal' ? "flex-row items-center px-3" : "flex-col"
    )}>
      {items.map((item) => (
        <div key={item.label} className={cn(
          "flex items-center gap-3",
          orientation === 'vertical' && "p-2 rounded-lg bg-card/50 border border-border/40"
        )}>
          {orientation === 'vertical' && (
            <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg border border-current/20", item.bgColor, item.color)}>
              <item.icon className="h-4 w-4" />
            </div>
          )}
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-zinc-300 uppercase tracking-widest leading-none mb-1.5">{item.label}</span>
            <span className={cn(
